...
class uploadAuthorVersion extends BaseController
{
  public function index()
  {
    ...
    if ($file != NULL) {
      ...
      $data["file"] = [
        'path' => 'uploads/author/' . $articleID . '/' . $fileID . '/' . $articleID . '-' . $fileID . '-' . $count . '-RV.pdf'
      ];
      ...
      if ($this->articleRevisionFilesModel->update($articleRevisionFileID, $data['article_revision_file']) && $this->filesModel->update($fileID, $data['file'])) {
        $file->store('author/' . $articleID . '/' . $fileID, $data['article_revision_file']['file_name']);
      }

      return redirect()->to('author/submissionReview/' . $articleID);
    }
  }
}
